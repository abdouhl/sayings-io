...
export async function generateMetadata(
  ...
  return {
    ...
    keywords: ["authors", "quotes", "inspirational quotes", "famous authors", "wisdom"],
  }
}
  ...
  return (
    <div className="container mx-auto py-8 md:py-16 px-4">
      <div className="max-w-3xl mx-auto text-center mb-12">
        <h1 className="text-3xl md:text-4xl font-bold mb-4">{dict.authors.allAuthors}</h1>
        <p className="text-muted-foreground mb-8 max-w-2xl mx-auto">{dict.authors.description}</p>
      </div>

      {authors.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-muted-foreground">{dict.authors.noAuthors || "No authors found"}</p>
        </div>
      ) : (
          ...
          {authors.map((author) => (
            ...
            >
              <Card className="h-full flex flex-col overflow-hidden border-2 hover:border-primary/50 transition-colors">
                <div className={`h-3 bg-gradient-to-r ${getGradientColor(author.name)}`}></div>
                <CardContent className="pt-6 flex-1 flex flex-col items-center text-center">
                  ...
                  <h2 className="text-xl font-bold mb-2 group-hover:text-primary transition-colors">{author.name}</h2>
